...
function info()
 {
    let url1 = { url: `http://api.gaoqingdianshi.com/api/coin/info`, headers: JSON.parse(signheaderVal)}
      ...
      if (result.errCode == 0)
        ...
        return coinid = `${result.data.tempCoin[0].id}`
         }
    ...
    sy.get(url3, (error, response, data) => 
  {
    ...
    let url5 = { url: `http://api.gaoqingdianshi.com/api/coin/temp/exchange?id=${coinid}`, headers: JSON.parse(signheaderVal)}
